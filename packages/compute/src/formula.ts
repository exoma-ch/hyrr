...
/** Element symbol -> atomic number. Complete IUPAC table through Z=118.
 *
 *  Z=93…118 (transuranics) matter because nucl-parquet ships a subset of
 *  them (Np, Pu, Am, Cm, Bk, Cf, Es, Fm, Md, Db) as Z-named cross-section
 *  files (`{proj}_Z{Z}.parquet`) and the browser data-store needs to
 *  resolve Z → symbol for the Z-form fallback (#488, mirrors the Rust fix
 *  in PR #555). Keeping the map complete also lets `hasCrossSections`
 *  gate transuranic cells in `MaterialPopup`'s "Show Z>92" view correctly
 *  instead of throwing "Unknown element". */
